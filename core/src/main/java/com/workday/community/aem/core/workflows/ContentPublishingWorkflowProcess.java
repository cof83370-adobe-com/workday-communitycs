	                ...
	                paths.stream().filter(item -> resResolver.getResource(item) != null)
	                        .forEach(path -> {
	                            try {
	                            	...
	                            	log.error("AccessDeniedException occured in replicateBookNodes: {}", e.getMessage());
								} catch (ItemNotFoundException e) {
									log.error("ItemNotFoundException occured in replicateBookNodes: {}", e.getMessage());
								} catch (RepositoryException e) {
									log.error("RepositoryException occured in replicateBookNodes: {}", e.getMessage());
								}
	                        });
