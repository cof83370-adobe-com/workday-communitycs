...
public class ContentPublishingWorkflowProcess implements WorkflowProcess {
    ...
    /** The Constant log. */
    private static final Logger LOGGER = LoggerFactory.getLogger(ContentPublishingWorkflowProcess.class);
    ...
    public void updatePageProperties(String pagePath, Session jcrSession, ResourceResolver resResolver) {
        try {
            ...
            if (null != currentPage) {
                ...
                if (node != null && template.getPath().equalsIgnoreCase(WorkflowConstants.EVENT_TEMPLATE_PATH)) {
            ...
            LOGGER.error("RepositoryException occurred in updatePageProperties {}:", e.getMessage());
        }
    }
    ...
    public void replicatePage(Session jcrSession, String pagePath, ResourceResolver resResolver) {
        try {
            if (replicator != null) {
                ...
                LOGGER.debug("PAGE ACTIVATION STARTED");
                replicator.replicate(jcrSession, ReplicationActionType.ACTIVATE, pagePath);
            }
        } catch (ReplicationException e) {
            LOGGER.error("Exception occured while replicatePage method: {}", e.getMessage());
        }
    }
        ...
        try {
            if (replicator != null) {
                ...
                for (Map.Entry<String, Asset> entry : allref.entrySet()) {
                    ...
                    LOGGER.debug("Asset activation started for {}", assetPath);
                    replicator.replicate(jcrSession, ReplicationActionType.ACTIVATE, assetPath);
                }
            }
        } catch (ReplicationException e) {
            LOGGER.error("Exception occured while replicateAssets method: {}", e.getMessage());
        }
    }
                ...
                paths.stream().filter(item -> resResolver.getResource(item) != null)
                        .forEach(path -> {
                            try {
                                ...
                                LOGGER.error("Exception occured while replicating the book node and cause was: {}",
                                        exec.getMessage());
                            }
                        });
                LOGGER.debug("Replicate node for page {}", pagePath);
            }
        } catch (Exception exec) {
            LOGGER.error("Exception occured while replicating the: {} page from book node. Exception was: {} :",
                    pagePath,
                    exec.getMessage());
        }
    }
