...
public class MetadataImpl implements Metadata {
    ...
    private static final String PROP_USER_PROFILE_GIVENNAME = "./profile/givenName";

    /** The Constant PROP_USER_PROFILE_FAMILYNAME. */
    private static final String PROP_USER_PROFILE_FAMILYNAME = "./profile/familyName";
    ...
    private static final String PROP_POSTED_DATE = "postedDate";
    ...
    public String getAuthorName() {
        try {
            String fullName = "";
            if (null != currentPage.getProperties()) {
                String author = currentPage.getProperties().get(PROP_AUTHOR, String.class);
                if (null != author) {
                    String authorFullName = getFullNameByUserID(author);
                    fullName = StringUtils.isNotBlank(authorFullName) ? authorFullName : author;
                } else {
                    fullName = getFullNameByUserID(currentPage.getProperties().get(PROP_JCR_CREATED_BY, String.class));
                }
            }
    ...
    private String getFullNameByUserID(String userID) {
        ...
        try {
            ...
            if (null != authorizable && !authorizable.isGroup()) {
                String firstName = authorizable.getProperty(PROP_USER_PROFILE_GIVENNAME) != null
                        ? authorizable.getProperty(PROP_USER_PROFILE_GIVENNAME)[0].getString()
                        : null;
                String lastName = authorizable.getProperty(PROP_USER_PROFILE_FAMILYNAME) != null
                        ? authorizable.getProperty(PROP_USER_PROFILE_FAMILYNAME)[0].getString()
                        : null;
    ...
    public Date getPostedDate() {
        return currentPage.getProperties().get(PROP_POSTED_DATE, Date.class);
    }
    ...
    public Date getUpdatedDate() {
        return currentPage.getProperties().get(PROP_UPDATED_DATE, Date.class);
    }
}
