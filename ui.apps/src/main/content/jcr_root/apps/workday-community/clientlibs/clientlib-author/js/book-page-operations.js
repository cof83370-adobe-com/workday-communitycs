...
(function (document, $, ns) {
  ...
  $(document).on('dialog-ready', function () {
    const CONSTANTS = {
      BOOK_COMP_CLASS: '.book-comp-class',
      CORAL_MULTI_CLASS: 'coral-multifield',
      CORAL_MULTI_INPUT: '.coral-Form-field',
      CQ_DIALOG_SUBMIT_CLASS: '.cq-dialog-submit'
    }

    // Only do custom logic if we're using our book component. 
    if ($(CONSTANTS.BOOK_COMP_CLASS).length > 0) {
      // on submit of the book component that will be registered for click event.
      $(CONSTANTS.BOOK_COMP_CLASS).parents().find(CONSTANTS.CQ_DIALOG_SUBMIT_CLASS).click(function () {
        //Check Validators for coral mutlifield and coral multifield inputs
        if ($(CONSTANTS.BOOK_COMP_CLASS).find(CONSTANTS.CORAL_MULTI_CLASS).checkValidity()) {
          ...
          function validateForm() {
            let valid = true;
            /* Select the form fields, for multifield form */
            const $formFields = $form.find(CONSTANTS.CORAL_MULTI_INPUT);
